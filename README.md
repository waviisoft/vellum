...
rejects three shapes the spec bans: a `Background:` block, because scenarios
are self-contained and shared setup belongs in the harness as a compound step;
a second `Feature:` in one fence, because a fence is one Gherkin document and a
stock Cucumber parser stops at the second; and a scenario that parses and can
never run, such as a `Scenario Outline` with no `Examples` rows, which reads as
coverage while pinning nothing. Prints one line per finding as
